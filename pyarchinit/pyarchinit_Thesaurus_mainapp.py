...
##		self.comboBox_sito.addItems(sito_vl)
	...
	def data_error_check(self):
		...
		if EC.data_is_empty(unicode(self.comboBox_sigla.currentText())) == 0:
			QMessageBox.warning(self, "ATTENZIONE", "Campo Sigla \n Il campo non deve essere vuoto",  QMessageBox.Ok)
			test = 1

		if EC.data_is_empty(unicode(self.comboBox_sigla_estesa.currentText())) == 0:
			QMessageBox.warning(self, "ATTENZIONE", "Campo Sigla estesa \n Il campo non deve essere vuoto",  QMessageBox.Ok)
			test = 1

		if EC.data_is_empty(unicode(self.comboBox_tipologia_sigla.currentText())) == 0:
			QMessageBox.warning(self, "ATTENZIONE", "Tipologia sigla. \n Il campo non deve essere vuoto",  QMessageBox.Ok)
			test = 1

		if EC.data_is_empty(unicode(self.comboBox_nome_tabella.currentText())) == 0:
			QMessageBox.warning(self, "ATTENZIONE", "Campo Nome tabella \n Il campo non deve essere vuoto",  QMessageBox.Ok)
			test = 1

		if EC.data_is_empty(unicode(self.textEdit_descrizione_sigla.toPlainText())) == 0:
	...
	def insert_new_rec(self):
		try:
			...
			unicode(self.comboBox_nome_tabella.currentText()), 				#1 - nome tabella
			unicode(self.comboBox_sigla.currentText()), 							#2 - sigla
			unicode(self.comboBox_sigla_estesa.currentText()), 					#3 - sigla estesa
			unicode(self.textEdit_descrizione_sigla.toPlainText()),					#4 - descrizione
			unicode(self.comboBox_tipologia_sigla.currentText()))					#5 - tipologia sigla
...
##			erp.export_rel_pdf()
	...
	def fill_fields(self, n=0):
		...
		unicode(self.comboBox_sigla.setEditText(self.DATA_LIST[self.rec_num].sigla))								#1 - Sigla
		unicode(self.comboBox_sigla_estesa.setEditText(self.DATA_LIST[self.rec_num].sigla_estesa))			#2 - Sigla estesa
		unicode(self.comboBox_tipologia_sigla.setEditText(self.DATA_LIST[self.rec_num].tipologia_sigla))		#3 - tipologia sigla
		unicode(self.comboBox_nome_tabella.setEditText(self.DATA_LIST[self.rec_num].nome_tabella))		#4 - nome tabella
		unicode(unicode(self.textEdit_descrizione_sigla.setText(self.DATA_LIST[self.rec_num].descrizione)))	#5 - descrizione sigla
	...
	def set_LIST_REC_TEMP(self):
		...
		self.DATA_LIST_REC_TEMP = [
			...
			unicode(self.comboBox_sigla_estesa.currentText()),			#3 - sigla estesa
			unicode(self.textEdit_descrizione_sigla.toPlainText()), 		#4 - descrizione
			unicode(self.comboBox_tipologia_sigla.currentText())			#3 - tipologia sigla
		]
